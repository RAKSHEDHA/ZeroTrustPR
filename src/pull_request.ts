...
export async function handlePullRequest() {
  ...
  const securitySummary = `### 🛡️ Security Audit Posture
**Is Secure:** ${review.is_secure ? "✅ Yes (No Medium, High, or Critical threats found)" : "❌ No"}
**Threat Summary:** ${review.threat_summary}`;

  // Post review comments
  const mappedComments: AIComment[] = review.vulnerabilities.map((v) => {
    const language = v.file.split(".").pop() || "";
    const remediationCodeBlock = v.remediation_code.trim()
      ? `\n\n**Remediation:**\n\`\`\`${language}\n${v.remediation_code}\n\`\`\``
      : "";
    ...
    return {
      ...
      highlighted_code: "",
      header: v.vulnerability_type,
      content: `### 🚨 Security Vulnerability: ${v.vulnerability_type}
**Threat Level:** ${v.threat_level}

**Description:**
${v.description}${remediationCodeBlock}`,
      label: "security",
      critical: true,
    };
  });
...
async function submitReview(
  ...
  try {
    ...
    await octokit.pulls.submitReview({
      ...
      event: "COMMENT",
      body: (securitySummary ? securitySummary + "\n\n" : "") + buildReviewSummary(
        context,
        files,
        commits,
        lineComments,
